class Forme:
    ...
    def effacer(self):
        self.__canevas.delete(self.id)
    
    def deplacement(self, dx, dy):
        self.__canevas.move(self.id, dx, dy)
        self.__x += dx
        self.__y += dy
...
class Rectangle(Forme):
    def __init__(self, canevas, x, y, l, h, couleur):
        ...
        self.id = canevas.create_rectangle(x, y, x+l, y+h, fill=couleur)
        self.set_item(self.id)
        self.__l = l
        self.__h = h
    ...
    def redimension_par_points(self, x0, y0, x1, y1):
        ...
        self.__l = abs(x0 - x1)
        self.__h = abs(y0 - y1)
        self.get_cavenas().coords(self.id, x0, y0, x1, y1)

    def vers_svg(self):
        return f'<rect x="{self.get_pos()[0]}" y="{self.get_pos()[1]}" width="{self.get_dim()[0]}" height="{self.get_dim()[1]}" style="fill:{self.couleur}" />'

class Ellipse(Forme):
    def __init__(self, canevas, x, y, rx, ry, couleur):
        Forme.__init__(self, canevas, x, y)
        self.id = canevas.create_oval(x-rx, y-ry, x+rx, y+ry, fill=couleur)
        self.set_item(self.id)
        self.__rx = rx
        self.__ry = ry
        self.couleur = couleur
    ...
    def vers_svg(self):
        return f'<ellipse cx="{self.get_pos()[0]}" cy="{self.get_pos()[1]}" rx="{self.get_dim()[0]}" ry="{self.get_dim()[1]}" style="fill:{self.couleur}" />'
