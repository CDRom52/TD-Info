...
class RequestHandler(http.server.SimpleHTTPRequestHandler):
  ...
  def send_ponctualite(self):
    ...
    # on découpe le path en tranches et on ignore la 1ère tranche qui est vide
    path_slices = self.path.split('/')[1:]
    # si pas de paramètre => erreur pas de région
    if len(path_slices) <= 1 :
        ...
        region = unquote(path_slices[1])
        # On teste que la région demandée existe bien
        c.execute("SELECT DISTINCT Région FROM 'regularite-mensuelle-ter'")
        r = c.fetchall()
        if (region,) not in r:   # Rq: r est une liste de tuples à 1 seul élement
            print ('Erreur nom')
            self.send_error(404,f"{region} : nom de région inconnu")   # Région non trouvée -> erreur 404
            return None
