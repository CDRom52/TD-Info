    ...
    def suppression_forme(self, x, y):
        for f in self.__formes[::-1]:
            if f.contient_point(x, y):
                ...
                break
    def save(self):
        with open("Draw.svg", "w") as filout:
            filout.write("<svg viewBox=\"0 0 300 200\" xmlns=\"http://www.w3.org/2000/svg\">\n")
            for f in self.__formes:
                if f.__class__ == Rectangle:
                    filout.write("  <rect x=\"{}\" y=\"{}\" width=\"{}\" height=\"{}\" style=\"fill: {}\"/>\n".format(f.x, f.y, f.get_l(), f.get_h(), f.get_c()))
                if f.__class__ == Ellipse:
                    filout.write("  <ellipse cx=\"{}\" cy=\"{}\" rx=\"{}\" ry=\"{}\" style=\"fill: {}\"/>\n".format(f.x, f.y, f.get_rx(), f.get_ry(), f.get_c()))
            filout.write("</svg>\n")
...
class FenPrincipale(Tk):
    def __init__(self):
        ...
        boutonSaveSvg = Button(barreOutils, text="Save svg")
        boutonSaveSvg.pack(side=LEFT, padx=5, pady=5)
        boutonQuitter = Button(barreOutils, text="Quitter")
        boutonQuitter.pack(side=LEFT, padx=5, pady=5)
        self.__canevas = ZoneAffichage(self, 300, 200)
        ...
        boutonSaveSvg.config(command=self.__canevas.save)
